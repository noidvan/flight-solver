...
fn bench_throughput_one<const N: usize, const P: usize>(
    ...
    for phase in 0..2 {
        ...
        for r in 0..iters {
            match solver {
                "standard" => {
                    ...
                    for i in 0..N_STEPS {
                        rls.update(&regs[i], &obs[i]);
                    }
                    if phase == 1 && r == iters - 1 {
                        x00 = rls.params()[(0, 0)];
                    }
                }
                "inverse_qr" => {
        ...
        if phase == 1 {
            ...
            println!(
                "{},{},{},{:.1},{:.1},{:.8e}",
                label, N_STEPS, solver, ns_seq, ns_step, x00
            );
        }
    }
}
...
fn run_scenario<const N: usize, const P: usize>(
    ...
    for (step, (a_arr, y_arr)) in data.iter().enumerate() {
        ...
        let mut a64 = [0.0f64; N];
        for i in 0..N {
            a64[i] = a_arr[i] as f64;
        }
        let mut y64 = [0.0f64; P];
        for i in 0..P {
            y64[i] = y_arr[i] as f64;
        }
    ...
    // High gamma: catastrophic cancellation in covariance form
    {
        ...
        let d: Vec<_> = (0..1000)
            .map(|_| {
                ...
                let y = [(tx[0][0] * a[0] as f64
                    + tx[1][0] * a[1] as f64
                    + tx[2][0] * a[2] as f64
                    + tx[3][0] * a[3] as f64
                    + 0.01 * rng.next_f32() as f64) as f32];
                (a, y)
            })
            .collect();
    ...
    // Low excitation burst: quiet then loud
    {
        ...
        let d: Vec<_> = (0..1000)
            .map(|step| {
                ...
                let mut y = [0.0f32; 3];
                for j in 0..3 {
                    y[j] = (tx[0][j] * a[0] as f64
                        + tx[1][j] * a[1] as f64
                        + 0.001 * rng.next_f32() as f64) as f32;
                }
                (a, y)
            })
            .collect();
